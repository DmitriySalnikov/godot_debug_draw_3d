...
#include "stats_3d.h"
#include "common/raw_data_buffer.h"
...
inline void DelayedRenderer::_update(double exp_time, bool is_vis) {
	expiration_time = exp_time;
	is_used_one_time = false;
	is_visible = is_vis;
}

inline bool DelayedRenderer::is_expired() const {
	return expiration_time > 0 ? false : is_used_one_time;
}

inline void DelayedRenderer::update_expiration(double delta) {
	if (!is_expired()) {
...
bool DelayedRendererInstance::update_visibility(const std::vector<std::vector<Plane> > &t_frustums, const GeometryPoolDistanceCullingData &t_distance_data, bool _skip_expiration_check) {
	return update_visibility_internal(bounds, t_frustums, t_distance_data, _skip_expiration_check);
}
...
bool DelayedRendererLine::update_visibility(const std::vector<std::vector<Plane> > &t_frustums, const GeometryPoolDistanceCullingData &t_distance_data, bool _skip_expiration_check) {
	return update_visibility_internal(bounds, t_frustums, t_distance_data, _skip_expiration_check);
}
...
PackedFloat32Array GeometryPool::get_raw_data(InstanceType _type) {
	...
	thread_local static auto temp_buffer = GDContainerBigRawBuffer<PackedFloat32Array, 128 * 1024, (int)InstanceType::ALL * 60>();
	...
	if (buffer_size > 0) {
		...
		auto write_data = [&last_added, &w](std::shared_ptr<DelayedRendererInstance> &o) {
...
void GeometryPool::fill_lines_data(Ref<ArrayMesh> _ig) {
	...
	thread_local static auto verices_buffer = GDContainerBigRawBuffer<PackedVector3Array, 64 * 1024>();
	thread_local static auto colors_buffer = GDContainerBigRawBuffer<PackedColorArray, 64 * 1024>();
	...
	auto append_lines = [&](const std::shared_ptr<DelayedRendererLine> &o) {
...
void GeometryPool::for_each_instance(const std::function<void(std::shared_ptr<DelayedRendererInstance>)> &_func) {
...
void GeometryPool::for_each_line(const std::function<void(std::shared_ptr<DelayedRendererLine>)> &_func) {
	for (size_t i = 0; i < lines.used_instant; i++) {
...
void GeometryPool::add_or_update_instance(InstanceType _type, real_t _exp_time, const Transform3D &_transform, const Color &_col, const SphereBounds &_bounds, const std::function<void(std::shared_ptr<DelayedRendererInstance>)> &_custom_upd) {
	std::shared_ptr<DelayedRendererInstance> inst = instances[(int)_type].get(_exp_time > 0);
	inst->update(_exp_time, _type, _transform, _col, _bounds);
	if (_custom_upd)
		_custom_upd(inst);
}

void GeometryPool::add_or_update_line(real_t _exp_time, const std::vector<Vector3> &_lines, const Color &_col, const std::function<void(std::shared_ptr<DelayedRendererLine>)> _custom_upd) {
	std::shared_ptr<DelayedRendererLine> inst = lines.get(_exp_time > 0);
